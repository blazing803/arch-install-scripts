...
mkswap "${DRIVE}1" || error "Failed to format swap partition ${DRIVE}1"
mkfs.ext4 "${DRIVE}2" || error "Failed to format root partition ${DRIVE}2"

# Mount the root partition
mount "${DRIVE}2" /mnt || error "Failed to mount root partition ${DRIVE}2"

# Activate swap
swapon "${DRIVE}1" || error "Failed to activate swap partition ${DRIVE}1"
