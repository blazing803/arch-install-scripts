...
mkfs.fat -F32 "${DRIVE}1" || error "Failed to format EFI partition ${DRIVE}1"
mkswap "${DRIVE}2" || error "Failed to format swap partition ${DRIVE}2"
mkfs.ext4 "${DRIVE}3" || error "Failed to format root partition ${DRIVE}3"

# Mount the partitions
mount "${DRIVE}3" /mnt || error "Failed to mount root partition ${DRIVE}3"
mkdir -p /mnt/boot/efi
mount "${DRIVE}1" /mnt/boot/efi || error "Failed to mount EFI partition ${DRIVE}1"
swapon "${DRIVE}2" || error "Failed to activate swap partition ${DRIVE}2"
...
arch-chroot /mnt bash -s <<EOF || error "Failed to chroot into installed system"
